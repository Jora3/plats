...
require_once "../../../controllers/par-categorie-controller.php";
...
<div class="grid_6">
    ...
    <div class="menu">
        <?php
        for ($i = 0; $i < $fin; $i++) {
            $categorie = $categories1[$i];
            ?>
            <h4 style="font-size: 24px;text-decoration: underline;"><?php if ($categorie instanceof \model\Categorie) echo $categorie->getNomCategorie(); ?></h4><br>
            <?php
            $plats1 = $plats[$i];
            $fin1 = count($plats1);
            for ($j = 0; $j < $fin1; $j++) {
